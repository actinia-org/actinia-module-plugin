...
from actinia_module_plugin.core.templates.utils import isGlobalTemplate
...
def _expand_nested_exported_results(pc_template_list_items):
    """
    Expaned nested list items when a nested actinia module is called
    """
    # find nested modules which may have an export on their own
    # TODO no recursion (finds exports only one level down)
    new_pc_template_list_items = []
    for i in pc_template_list_items:
        module_name = i.get("module")
        # check if it is a global template, then add pc list of that tpl
        if isGlobalTemplate(module_name):
            nested_pc_template, _ = render_template(module_name)
            new_pc_template_list_items += nested_pc_template["template"][
                "list"
            ]
        # user_templates are not checked yet and treated as normal modules
        # meaning that they are added but not checked for nested exporter
        else:
            new_pc_template_list_items.append(i)
    return new_pc_template_list_items
...
def _generate_exported_results_including_nested_items(pc_template_list_items):
    # Populate list items if there are nested modules
    new_pc_template_list_items = _expand_nested_exported_results(
        pc_template_list_items
    )
    # create results array out of list items
    return _generate_exported_results(new_pc_template_list_items)
...
def createActiniaModule(
    ...
    exported_results = []
    if returns == "export":
        exported_results = _generate_exported_results_including_nested_items(
            pc_template_list_items
        )
